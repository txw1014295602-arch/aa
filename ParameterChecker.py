...
class ParameterChecker:
    ...
    def create_sample_excel(self, file_path: str = '参数知识库.xlsx') -> None:
        """创建新版本双分表结构的示例参数知识库Excel文件，展示完整的嵌套验证场景"""
        
        # Sheet 1: 参数信息表 - 记录所有参数的基本信息
        param_info_data = [
            # 说明行
            {
                'MO名称': '=== 参数信息表说明 ===',
                'MO描述': '记录所有参数的基本信息，与验证规则表分离',
                '场景类型': '参数类型: single(单值)/multiple(多值开关组合)',
                '参数名称': '参数的中文名称，用于验证规则表中引用',
                '参数ID': '参数的英文ID，用于生成MOD命令',
                '参数类型': 'single/multiple，决定验证方式',
                '参数含义': '参数的详细说明和业务含义',
                '值描述': '仅多值参数使用，说明各开关含义'
            },
            # 实际参数定义
            ...
            },
            {
                ...
                '场景类型': '空域配置',
                '参数名称': '波束开关组合',
                '参数ID': 'BeamSwitchCombination',
                '参数类型': 'multiple',
                '参数含义': '多个波束开关的组合状态配置',
                '值描述': 'beam1:第1号波束开关状态(开/关); beam2:第2号波束开关状态(开/关); beam3:第3号波束开关状态(开/关)'
            },
            {
        ...
        # Sheet 2: 验证规则表 - 完整的嵌套验证场景示例
        validation_rules_data = [
            # 说明行
            {
                '校验ID': '=== 验证规则表说明 ===',
                '校验类型': '漏配/错配，每行只能是其中一种',
                'MO名称': '目标MO名称，必须在参数信息表中存在',
                '参数名称': '目标参数名称，必须在参数信息表中存在',
                '条件表达式': '筛选条件: (参数名1=值1 and 参数名2=值2) or (参数名3>值3 and 参数名2!=值2)',
                '期望值': '单值参数直接填值，多值参数格式: k1:开&k2:关&k3:开',
                '继续校验': '下一步校验的校验ID，支持嵌套调用',
                '错误描述': '校验失败时的详细描述信息'
            },
            ...
            # === 场景1: 基础漏配检查 ===
            {
                ...
                '条件表达式': '',  # 无条件，直接检查
                '期望值': '100',
                '继续校验': 'ERROR_001',  # 漏配检查通过后，继续错配检查
                '错误描述': '缺少跟踪区码为100的小区配置'
            },
            ...
            # === 场景2: 嵌套错配检查 - 接上面的漏配检查 ===
            {
                ...
                'MO名称': 'NRDUCELL',
                '参数名称': '小区半径(米)',
                '条件表达式': '跟踪区码=100',  # 只对跟踪区码为100的小区检查
                '期望值': '500',
                '继续校验': 'ERROR_002',  # 继续下一个错配检查
                '错误描述': '跟踪区码为100的小区，半径应配置为500米'
            },
            ...
            # === 场景3: 多参数联合错配检查 ===
            {
                ...
                'MO名称': 'NRDUCELL',
                '参数名称': '最大传输功率',
                '条件表达式': '跟踪区码=100 and 小区半径(米)=500',  # 多参数联合条件
                '期望值': '43',
                '继续校验': 'ERROR_003',  # 继续多值参数检查
                '错误描述': '满足条件的小区，最大传输功率应为43dBm'
            },
            ...
            # === 场景4: 复杂条件多值参数检查 ===
            {
                ...
                'MO名称': 'NRDUCELLBEAM',
                '参数名称': '波束开关组合',
                '条件表达式': '(小区半径(米)>=500 and 最大传输功率>=40) or (跟踪区码!=200 and 小区状态=激活)',
                '期望值': 'beam1:开&beam2:关&beam3:开',
                '继续校验': 'MISS_002',  # 错配检查后继续漏配检查
                '错误描述': '满足功率和覆盖条件时，beam1和beam3应开启，beam2应关闭'
            },
            ...
            # === 场景5: 反向嵌套 - 错配检查后的漏配检查 ===
            {
                ...
                'MO名称': 'NRCELLFREQRELATION',
                '参数名称': '连接态频率优先级',
                '条件表达式': '小区状态=激活 and 最大传输功率>40',
                '期望值': '5',
                '继续校验': 'ERROR_004',  # 再次回到错配检查
                '错误描述': '激活状态且高功率小区必须配置频率优先级为5'
            },
            ...
            # === 场景7: 独立的复杂OR条件示例 ===
            {
                ...
                'MO名称': 'NRDUCELL',
                '参数名称': '小区半径(米)',
                '条件表达式': '(跟踪区码=100 and 小区状态=激活) or (跟踪区码=200 and 最大传输功率>45) or (小区状态!=激活 and 连接态频率优先级>=3)',
                '期望值': '800',
                '继续校验': '',
                '错误描述': '满足任一复杂条件组合时，小区半径应为800米'
            }
        ]
        ...
        with pd.ExcelWriter(file_path, engine='openpyxl') as writer:
            ...
            df_rules.to_excel(writer, sheet_name='验证规则', index=False)
            
        logger.info(f"新版本双分表参数知识库已生成: {file_path}")
        logger.info("===== 新版本核心功能 =====")
        logger.info("1. 双分表设计：参数信息与验证规则完全分离")
        logger.info("2. 嵌套验证链：MISS_001→ERROR_001→ERROR_002→ERROR_003→MISS_002→ERROR_004")
        logger.info("3. 复杂条件支持：(param1=value1 and param2=value2) or (param3>value3)")
        logger.info("4. 多值参数处理：beam1:开&beam2:关&beam3:开 格式")
        logger.info("5. 交替校验：漏配↔错配 无限嵌套调用")
        logger.info("===== 验证场景示例 =====")
        logger.info("• 先检查是否缺少跟踪区码100的配置(漏配)")
        logger.info("• 再检查跟踪区码100小区的半径是否为500米(错配)")
        logger.info("• 继续检查功率配置、波束开关组合等(多层嵌套)")
        logger.info("• 支持复杂OR条件的独立校验规则")
